...
2.  Use storage-tier-list to show the existing storage tier(s), e.g.:
    ...
    Ensure the information is accurate.
3.  Use system storage-tier-add ceph_cluster <tiername> to add a new storage tier, e.g.:
...
4.  To associate OSDs (where tier-uuid is the uuid of the new storage tier taken from system storage-tier-list)
...
- Ideally this test would be run on all supported ceph configs: All-in-One Simplex,
